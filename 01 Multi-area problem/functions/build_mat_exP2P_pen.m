...
for i=1:p.n
    ...
    p.m.beq{i} = sparse([beq1;beq2;beq3;beq4;beq5]);
    ...
    if p.en.st_un(i) == 1
        ...
        Am = p.en.a_st(i);
        Bm = [0 p.en.eta_ch(i) -1/p.en.eta_dh(i) 0 0 0 0 0 zeros(1,p.en.noN(i)) zeros(1,p.ny(i)+p.nz(i))];
...
%     b4 = kron(ones(p.h,1),pt_m);
    ...
    p.m.bineq{i} = sparse([b1;b2;b3;b4;b5;b6;b7;b8;bp]);
