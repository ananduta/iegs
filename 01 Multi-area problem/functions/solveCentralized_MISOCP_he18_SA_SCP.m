function [p,o,scp]= solveCentralized_MISOCP_he18_SA_SCP(p,scp,a,k)
...
%    p = build_mat_iegs_he18(p);  
    ...
    for ii = 1:n
        ...
        Ai = sparse(zeros(size(At{ii},1),idEnd(end)));
        Ai(:,idStart(ii):idEnd(ii)) = At{ii};
        bi = bt{ii};
...
%         end
    ...
    for ii = 1:n
        ...
        if p.en.dgu_un(i) == 1
            for h = 1:p.h
                id_p_dg_ih = idStart(ii)-1+p.nu(ii)*(h-1)+p.id_dg;
                id_p_gu_ih = idStart(ii)-1+p.nu(ii)*(h-1)+p.id_gu;
...
%    o.res = res;
    ...
    scp.z(a,k+1) = res.objval;
    scp.gapZ(a,k+1) = scp.z(k+1) - scp.z(k);
    
    scp.gapS(a,k+1) = sum_pen;
