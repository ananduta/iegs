function p = build_mat_iegs_he18(p)
...
for i=1:p.n
    ...
    E1a(1,p.id_gu) = 0;
    E1a(1,p.id_th) = 0;
    E1a(1,p.id_th+1:p.id_th+p.en.noN(i)) = 1;
    %E1a(1,p.id_th+1:p.nx(i)) = -ones(1,p.en.noN(i));
...
%     E1b(1,p.id_mg) = 1;
%     E1b(1,p.id_eg:p.nx(i)) = -1*ones(1,p.en.noN(i)+1);
...
%     end
    ...
    % MODIFY INTO QUADRATIC CONSTRAINTS
    if p.en.dgu_un(i) == 1
%         E2 = zeros(1,p.nu(i));
...
%           -p.en.v_min(i)*ones(p.h,1)];
    ...
    %if p.gn.gs(i) == 1
        A5 = [ p.m.Sgs{i};
              -p.m.Sgs{i}];
        b5 = [ p.gn.gs_max(i)*ones(p.h,1);
              -p.gn.gs_min(i)*ones(p.h,1)];
    %end
    
    % flow limits (15)
    % and non-negativity of s
    % and lower bound of gamma
    for jj = 1:p.gn.noN(i)
        j = p.gn.N{i}(jj);
...
%                    zeros(p.h,1)];
    ...
    p.m.Aineq{i} = sparse([A1;A2;A3;A4;A5;A7;A8;Ap;A6]);
    p.m.bineq{i} =       ([b1;b2;b3;b4;b5;b7;b8;bp;b6]);
